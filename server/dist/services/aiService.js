...
exports.generateLinkedInReport = exports.generateGithubReport = exports.generateMockRecruiterFeedback = exports.generateBrandContent = exports.generateChatResponse = exports.askGemini = void 0;
...
const askGemini = async (prompt, systemInstruction) => {
    if (gemini_1.isGeminiConfigured && gemini_1.googleGenAI) {
        try {
            const model = gemini_1.googleGenAI.getGenerativeModel({
...
const generateChatResponse = async (userId, messages, contextDocs) => {
    const user = await User_1.default.findOne({ firebaseUid: userId });
...
- Style patterns: ${writingStyle?.patterns?.join('; ') || 'Starts with a hook; Uses bullet points'}`;
    ...
    return (0, exports.askGemini)(fullPrompt, systemInstruction);
};
exports.generateChatResponse = generateChatResponse;
// Generates personal branding content (LinkedIn, Twitter, Resume, Bios)
const generateBrandContent = async (userId, contentType, prompt) => {
    const user = await User_1.default.findOne({ firebaseUid: userId });
    const writingStyle = user?.writingStyleProfile;
    const displayName = user?.displayName || 'the user';
...
exports.generateBrandContent = generateBrandContent;
// Generates feedback for mock recruiter interviews
const generateMockRecruiterFeedback = async (role, messages) => {
    ...
    const prompt = `Review this technical/behavioral interview transcript for the role of "${role}":\n\n${transcript}\n\nProvide a detailed assessment. You MUST respond with a valid JSON block containing:
{
  ...
  "transcriptFeedback": string (overall review text)
    ...
    const aiResponse = await (0, exports.askGemini)(prompt, "You are a senior recruiter and technical interviewer. Always output response as a clean JSON object.");
    try {
...
exports.generateMockRecruiterFeedback = generateMockRecruiterFeedback;
// Generates Github analysis report
const generateGithubReport = async (repos) => {
    ...
    return (0, exports.askGemini)(prompt, "You are an expert technical director auditing code portfolios.");
};
exports.generateGithubReport = generateGithubReport;
// Generates LinkedIn analysis report
const generateLinkedInReport = async (profileData) => {
    const prompt = `Analyze this profile overview and suggest missing skills, resume enhancements, and networking paths:\n\n${JSON.stringify(profileData)}`;
    return (0, exports.askGemini)(prompt, "You are a senior career advisor optimizing profiles for executive recruiting.");
};
exports.generateLinkedInReport = generateLinkedInReport;
