 ...
 */
export const analyzeStylometrics = (text: string) => {
 ...
 */
export const learnWritingStyle = async (userId: string): Promise<StyleProfile> => {
  ...
  const localAnalysis = analyzeStylometrics(combinedText);
...
Provide a stylized profile. You MUST respond with a valid JSON block containing:
{
  ...
  const aiResponse = await askGemini(prompt, "You are a professional computational linguist and writing style assessor.");
  
  let tone = 'Professional, informative, and engaging';
  let patterns: string[] = ['Starts with a hook', 'Uses bullet points for readability', 'Ends with a question'];
  let samplePost = '';
